<template>
  <div>
    <group title="家庭成员详细信息" labelWidth="6.5rem" gutter="0" labelMarginRight="1rem">
      <popup-picker title="称谓" :data="cwlist" v-model="person.subty" :columns="1" show-name
...
<script>
  ...
  export default {
    ...
    created() {
      let jtlist = JSON.parse(this.$route.query.pj);
      this.person = jtlist.fam;
      this.person.subty = [jtlist.fam.subty];
      this.cwlist = this.modiData(jtlist.cw);
    },
    methods: {
      modiData(data) {
      ...
      saveinfo() {
        ...
        params.append('id', _that.person.id || '');
